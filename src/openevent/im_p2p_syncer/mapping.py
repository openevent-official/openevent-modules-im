...
class P2PMappingIndex:
    ...
    def channel_for_provider_session(self, provider: str, session_id: str) -> int:
        return self._session_channel[(provider, session_id)]
    ...
    def sender_external_user_id(
        ...
        if entry is None:
            raise KeyError(f"principal {principal} is not mapped in channel {channel_id}")
        if identity_type is not None and entry.identity_type != identity_type:
            raise KeyError(
                f"principal {principal} is not mapped as {identity_type} in channel {channel_id}"
            )
    ...
    def principal_for_external_user(
        ...
        if entry is None:
            raise KeyError(
                f"external {identity_type} {external_user_id} is not mapped in channel {channel_id}"
            )
    ...
    def _build(self) -> None:
        for item in self._config.mappings:
        ...
        for channel_id, entries in self._by_channel.items():
            ...
            if len(principals) != 2 or len(entries) != 2:
                raise ConfigError(
                    f"p2p channel {channel_id} must have exactly two distinct principals"
                )
            if identity_types != {"user", "bot"}:
                raise ConfigError(f"p2p channel {channel_id} must have one user and one bot")
            for item in entries:
